...
const getOrCreateUID = (): string => {
  const uuidKey = "check-api-spotlight-uuid"
  const existingUID = localStorage.getItem(uuidKey)
  if (existingUID) return existingUID

  const newUID = uuidv4()
  localStorage.setItem(uuidKey, newUID)
  return newUID
}
...
const bearerInterceptor = (
  ...
  config.headers.set("uid", getOrCreateUID())
  return config
}
